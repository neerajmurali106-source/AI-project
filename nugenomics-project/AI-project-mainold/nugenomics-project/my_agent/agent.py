...
from .agents import nugen_agent, well_agent

try:
    nugen_tool = AgentTool.from_agent(nugen_agent)
    well_tool = AgentTool.from_agent(well_agent)
except AttributeError:
    try:
        nugen_tool = AgentTool(agent=nugen_agent)
        well_tool = AgentTool(agent=well_agent)
    except TypeError:
        # Oldest fallback: positional agent
        nugen_tool = AgentTool(nugen_agent)
        well_tool = AgentTool(well_agent)
...
router_agent = LlmAgent(
    ...
    instruction=(
        ...
        "→ If the question is about genetics, DNA, health, or wellness, route it to the 'well_agent'.\n"
