...
def search_faq_text(query: str) -> str:
    ...
    for item in FAQ_DATA:
        ...
        if score > 0.18 or any(term in text for term in search_terms):
            results.append(item)

    if not results:
        return (
            "The information about your query could not be found directly in our FAQ. "
            "Please contact Nugenomics support for help via info@nugenomics.in."
        )

    formatted_results = [
        f"Answer: {item['content'].split('Answer: ')[-1]}\n[Source: {item['source']}]"
